    ...
    async fazerLogin(req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (email === req.body.email && await bcrypt.compare(req.body.senha, senha)) {
                ...
                logger.info(`[Login bem-sucedido] Id usuário: ${_id} - Method: ${req.method} - URL: ${req.url} - { Host: ${req.headers.host} - User-agent: ${req.headers["user-agent"]} }`);
                
                return res.status(200).json({ auth: true, token: token, usuario: { email: email } });
            }
            
            logger.error(`[Login malsucedido] Id usuário: ${_id} - Method: ${req.method} - URL: ${req.url} - { Host: ${req.headers.host} - User-agent: ${req.headers["user-agent"]} }`);
